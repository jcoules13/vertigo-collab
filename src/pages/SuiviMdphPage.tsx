...
const STEPS = [
  { key: 'brouillon', label: 'Brouillon', color: 'bg-gray-400', textColor: 'text-gray-700 dark:text-gray-300' },
  { key: 'en_cours', label: 'En cours', color: 'bg-blue-500', textColor: 'text-blue-700 dark:text-blue-400' },
  { key: 'a_relire', label: 'À relire', color: 'bg-amber-500', textColor: 'text-amber-700 dark:text-amber-400' },
  { key: 'pret', label: 'Prêt', color: 'bg-green-500', textColor: 'text-green-700 dark:text-green-400' },
  { key: 'envoye', label: 'Envoyé', color: 'bg-teal-500', textColor: 'text-teal-700 dark:text-teal-400' },
] as const
...
export default function SuiviMdphPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white dark:from-gray-900 dark:to-gray-950 flex flex-col items-center px-4 py-8 sm:py-12">
      {/* Header */}
      <div className="w-full max-w-lg flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">Vertigo Com' Handicap</h1>
            <p className="text-xs text-gray-500 dark:text-gray-400">Suivi de votre dossier MDPH</p>
          </div>
        </div>
        <ThemeToggle />
      </div>
        ...
        {!result ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 sm:p-8">
            <div className="text-center mb-6">
              ...
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Consultez l'avancement de votre dossier</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 text-lg font-mono tracking-wider border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-center placeholder:text-gray-400 dark:placeholder:text-gray-500"
                  maxLength={22}
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Voici l'état de votre dossier MDPH.</p>
            </div>

            {/* Status stepper */}
            {result.mdph ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">État du formulaire</h3>
                      ...
                      <div key={step.key} className="flex items-center flex-1">
                        <div className="flex flex-col items-center flex-1">
                          <div className={`
                            ...
                            ${isPast || isCurrent ? step.color : 'bg-gray-200 dark:bg-gray-600'}
                            ${isCurrent ? 'ring-4 ring-offset-2 ring-teal-300 dark:ring-teal-600 dark:ring-offset-gray-800 animate-pulse' : ''}
                          `}>
                            {isPast ? '✓' : i + 1}
                          </div>
                          <span className={`text-[10px] sm:text-xs mt-1.5 font-medium text-center leading-tight ${
                            isCurrent ? step.textColor + ' font-bold' : isFuture ? 'text-gray-400 dark:text-gray-500' : 'text-gray-600 dark:text-gray-400'
                          }`}>
                            {step.label}
                          </span>
                        </div>
                        {i < STEPS.length - 1 && (
                          <div className={`h-0.5 flex-1 mx-1 ${i < currentStepIndex ? 'bg-teal-400' : 'bg-gray-200 dark:bg-gray-600'}`} />
                        )}
                      </div>
                    )
                  })}
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Aucun formulaire MDPH en cours pour ce dossier.</p>
              </div>
            )}
              ...
              {result.prochain_rdv ? (
                <div className="border-l-4 border-teal-500 pl-4 py-2">
                  ...
                  {result.prochain_rdv.lieu && (
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">📍 {result.prochain_rdv.lieu}</p>
                  )}
                </div>
              ) : (
                <p className="text-sm text-gray-500 dark:text-gray-400">Aucun rendez-vous programmé pour le moment.</p>
              )}
            </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                ...
                {result.documents_a_preparer && (
                  <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    <p className="text-xs font-medium text-amber-700 dark:text-amber-300 mb-1">À préparer pour votre prochain rendez-vous :</p>
                    <p className="text-sm text-amber-900 dark:text-amber-200 whitespace-pre-line">{result.documents_a_preparer}</p>
                  </div>
                )}

                {result.documents_mdph && (
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Documents obligatoires MDPH :</p>
                    {Object.entries(result.documents_mdph).map(([key, value]) => (
                      <div key={key} className="flex items-center gap-2">
                        ...
                        <span className={`text-sm ${value ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'}`}>
                          {DOC_LABELS[key] || key}
                        </span>
                      </div>
                    ))}
            ...
            <button
              onClick={handleReset}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            >
